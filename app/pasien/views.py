...
from app.models import Diagnosa, Label, Pasien, Pilihan, Role, User
from app.pasien.forms import PasienForm, PasienInputForm
...
def index():
    """Index Pasien page."""
    pasien = Pasien.query.join(User, Pasien.user == User.id).add_columns(
        Pasien.id, User.first_name.label('first_name'),
        User.last_name.label('last_name'), Pasien.k1, Pasien.k2, Pasien.k3,
        Pasien.k4, Pasien.k5, Pasien.k6, Pasien.k7).all()
    pilihans = Pilihan.query.all()
    labels = Label.query.all()
    return render_template('pasien/index.html',
                           pasien=pasien,
                           pilihans=pilihans,
                           labels=labels)
...
def new_data():
    ...
    if form.validate_on_submit():
        if Pasien.query.filter_by(user=form.user.data).first():
            ...
            datapasien = Pasien(user=form.user.data,
                                k1=form.k1.data,
                                k2=form.k2.data,
                                k3=form.k3.data,
                                k4=form.k4.data,
            ...
            flash(
                'Data pasien {} successfully created & diagnosa'.format(
                    name.full_name()), 'form-success')
...
def input():
    ...
    if form.validate_on_submit():
        if Pasien.query.filter_by(user=current_user.id).first():
            ...
            flash(
                'Data pasien {} successfully changed'.format(name.full_name()),
                'form-success')
        else:
            datapasien = Pasien(user=current_user.id,
                                k1=form.k1.data,
                                k2=form.k2.data,
                                k3=form.k3.data,
                                k4=form.k4.data,
